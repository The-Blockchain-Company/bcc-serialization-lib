...
#[wasm_bindgen]
#[derive(Debug, Clone, Eq, Ord, PartialEq, PartialOrd)]
pub struct ColeAddress(pub (crate) ExtendedAddr);
#[wasm_bindgen]
impl ColeAddress {
    ...
    pub fn from_bytes(bytes: Vec<u8>) -> Result<ColeAddress, JsError> {
        let mut raw = Deserializer::from(std::io::Cursor::new(bytes));
        let extended_addr = ExtendedAddr::deserialize(&mut raw)?;
        Ok(ColeAddress(extended_addr))
    }
    /// returns the cole protocol magic embedded in the address, or mainnet id if none is present
    /// note: for bech32 addresses, you need to use network_id instead
    pub fn cole_protocol_magic(&self) -> u32 {
    ...
    pub fn from_base58(s: &str) -> Result<ColeAddress, JsError> {
        use std::str::FromStr;
    ...
    pub fn icarus_from_key(key: &Bip32PublicKey, protocol_magic: u32) -> ColeAddress {
        let mut out = [0u8; 64];
        out.clone_from_slice(&key.as_bytes());

        // need to ensure we use None for mainnet since Cole-era addresses omitted the network id
        let filtered_protocol_magic = if protocol_magic == NetworkInfo::mainnet().protocol_magic() { None } else { Some(protocol_magic) };
        ColeAddress(ExtendedAddr::new_simple(& XPub::from_bytes(out), filtered_protocol_magic))
    }
    ...
    pub fn to_address(&self) -> Address {
        Address(AddrType::Cole(self.clone()))
    }

    pub fn from_address(addr: &Address) -> Option<ColeAddress> {
        match &addr.0 {
            AddrType::Cole(cole) => Some(cole.clone()),
            _ => None,
        }
    }
}
...
impl Address {
    ...
    fn from_bytes_impl(data: &[u8]) -> Result<Address, DeserializeError> {
        ...
        // Copied from sophie.cddl:
        //
        // sophie payment addresses:
        // bit 7: 0
        // bit 6: base/other
        ...
        (|| -> Result<Self, DeserializeError> {
            ...
            let addr = match (header & 0xF0) >> 4 {
                ...
                // cole
                0b1000 => {
                    // note: 0b1000 was chosen because all existing Cole addresses actually start with 0b1000
                    // Therefore you can re-use Cole addresses as-is
                    match ColeAddress::from_bytes(data.to_vec()) {
                        Ok(addr) => AddrType::Cole(addr),
...
mod tests {
    ...
    #[test]
    fn cole_magic_parsing() {
        // mainnet address w/ protocol magic omitted
        let addr = ColeAddress::from_base58("Ae2tdPwUPEZ4YjgvykNpoFeYUxoyhNj2kg8KfKWN2FizsSpLUPv68MpTVDo").unwrap();
        assert_eq!(addr.cole_protocol_magic(), NetworkInfo::mainnet().protocol_magic());
        assert_eq!(addr.network_id().unwrap(), NetworkInfo::mainnet().network_id());

        // original Cole testnet address
        let addr = ColeAddress::from_base58("2cWKMJemoBaipzQe9BArYdo2iPUfJQdZAjm4iCzDA1AfNxJSTgm9FZQTmFCYhKkeYrede").unwrap();
        assert_eq!(addr.cole_protocol_magic(), NetworkInfo::testnet().protocol_magic());
    ...
    fn parse_redeem_address() {
        assert!(ColeAddress::is_valid("Ae2tdPwUPEZ3MHKkpT5Bpj549vrRH7nBqYjNXnCV8G2Bc2YxNcGHEa8ykDp"));
        let cole_addr = ColeAddress::from_base58("Ae2tdPwUPEZ3MHKkpT5Bpj549vrRH7nBqYjNXnCV8G2Bc2YxNcGHEa8ykDp").unwrap();
        assert_eq!(cole_addr.to_base58(), "Ae2tdPwUPEZ3MHKkpT5Bpj549vrRH7nBqYjNXnCV8G2Bc2YxNcGHEa8ykDp");
        let cole_addr2 = ColeAddress::from_bytes(cole_addr.to_bytes()).unwrap();
        assert_eq!(cole_addr2.to_base58(), "Ae2tdPwUPEZ3MHKkpT5Bpj549vrRH7nBqYjNXnCV8G2Bc2YxNcGHEa8ykDp");
    }

    #[test]
    fn bip32_15_cole() {
        let cole_key = root_key_15()
            .derive(harden(44))
            .derive(harden(1815))
            .derive(harden(0))
            .derive(0)
            .derive(0)
            .to_public();
        let cole_addr = ColeAddress::icarus_from_key(&cole_key, NetworkInfo::mainnet().protocol_magic());
        assert_eq!(cole_addr.to_base58(), "Ae2tdPwUPEZHtBmjZBF4YpMkK9tMSPTE2ADEZTPN97saNkhG78TvXdp3GDk");
        assert!(ColeAddress::is_valid("Ae2tdPwUPEZHtBmjZBF4YpMkK9tMSPTE2ADEZTPN97saNkhG78TvXdp3GDk"));
        assert_eq!(cole_addr.network_id().unwrap(), 0b0001);

        let cole_addr_2 = ColeAddress::from_address(&Address::from_bytes(cole_addr.to_bytes()).unwrap()).unwrap();
        assert_eq!(cole_addr.to_base58(), cole_addr_2.to_base58());
    }
