...
/// Legacy Klarity algorithm
pub struct LegacyKlarity;
...
impl AsymmetricPublicKey for LegacyKlarity {
    type Public = XPub;
...
impl AsymmetricKey for LegacyKlarity {
    type Secret = LegacyPriv;
    type PubAlg = LegacyKlarity;
    ...
    fn secret_from_binary(data: &[u8]) -> Result<Self::Secret, SecretKeyError> {
        ...
        // this is because legacy Klarity wallets do not match the format
...
impl SecretKeySizeStatic for LegacyKlarity {
    const SECRET_KEY_SIZE: usize = XPRV_SIZE;
}

type XSig = ed25519_bip32::Signature<u8>;

impl VerificationAlgorithm for LegacyKlarity {
    type Signature = XSig;
...
impl SigningAlgorithm for LegacyKlarity {
...
mod test {
    ...
    #[quickcheck]
    fn sign_ok(input: (KeyPair<LegacyKlarity>, Vec<u8>)) -> bool {
        keypair_signing_ok(input)
    }
    #[quickcheck]
    fn sign_ko(input: (KeyPair<LegacyKlarity>, KeyPair<LegacyKlarity>, Vec<u8>)) -> bool {
