...
// https://github.com/The-Blockchain-Company/js-chain-libs
...
impl Bip32PrivateKey {
    ...
    /// 128-byte xprv a key format in Bcc that some software still uses or requires
...
#[wasm_bindgen]
pub struct LegacyKlarityPrivateKey(pub (crate) crypto::SecretKey<crypto::LegacyKlarity>);

#[wasm_bindgen]
impl LegacyKlarityPrivateKey {
    pub fn from_bytes(bytes: &[u8]) -> Result<LegacyKlarityPrivateKey, JsError> {
        crypto::SecretKey::<crypto::LegacyKlarity>::from_binary(bytes)
            .map_err(|e| JsError::from_str(&format!("{}", e)))
            .map(LegacyKlarityPrivateKey)
    }
