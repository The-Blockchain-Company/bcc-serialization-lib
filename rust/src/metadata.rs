...
pub struct AuxiliaryData {
    ...
    zerepoch_scripts: Option<ZerepochScripts>,
}
...
impl AuxiliaryData {
    ...
    pub fn zerepoch_scripts(&self) -> Option<ZerepochScripts> {
        self.zerepoch_scripts.clone()
    }

    pub fn set_zerepoch_scripts(&mut self, zerepoch_scripts: &ZerepochScripts) {
        self.zerepoch_scripts = Some(zerepoch_scripts.clone())
    }
}
...
// This conversion should match TxMetadataJsonSchema in bcc-node defined (at time of writing) here:
// https://github.com/The-Blockchain-Company/bcc-node/blob/master/bcc-api/src/Bcc/Api/MetaData.hs
...
pub enum MetadataJsonSchema {
    ...
    // Stricter than any TxMetadataJsonSchema in bcc-node but more natural for JSON -> Metadata
    NoConversions,
    ...
    // Corresponds to TxMetadataJsonSchema's TxMetadataJsonNoSchema in bcc-node
    BasicConversions,
    // Supports the annotated schema presented in bcc-node with tagged values e.g. {"int": 7}, {"list": [0, 1]}
    // Round-trip conversions are 100% consistent
    ...
    // Corresponds to TxMetadataJsonSchema's TxMetadataJsonDetailedSchema in bcc-node
    DetailedSchema,
}
...
impl cbor_event::se::Serialize for AuxiliaryData {
    fn serialize<'se, W: Write>(&self, serializer: &'se mut Serializer<W>) -> cbor_event::Result<&'se mut Serializer<W>> {
        // we still serialize using the sophie-jen era format as it is still supported
        // and it takes up less space on-chain so this should be better for scaling.
        // Plus the code was already written for sophie-jen anyway
        if self.metadata.is_some() && self.zerepoch_scripts.is_none()  {
            ...
            // new format with zerepoch support
            ...
            serializer.write_map(cbor_event::Len::Len(
                ...
                if self.zerepoch_scripts.is_some() { 1 } else { 0 }))?;
            ...
            if let Some(zerepoch_scripts) = &self.zerepoch_scripts {
                serializer.write_unsigned_integer(2)?;
                zerepoch_scripts.serialize(serializer)?;
            }
            Ok(serializer)
        }
    }
    ...
    fn deserialize<R: BufRead + Seek>(raw: &mut Deserializer<R>) -> Result<Self, DeserializeError> {
        (|| -> Result<_, DeserializeError> {
            match raw.cbor_type()? {
                    ...
                    while match len { cbor_event::Len::Len(n) => read < n as usize, cbor_event::Len::Indefinite => true, } {
                        match raw.cbor_type()? {
                            CBORType::UnsignedInteger => match raw.unsigned_integer()? {
                                ...
                                2 =>  {
                                    if zerepoch_scripts.is_some() {
                                        return Err(DeserializeFailure::DuplicateKey(Key::Uint(2)).into());
                                    }
                                    zerepoch_scripts = Some((|| -> Result<_, DeserializeError> {
                                        read_len.read_elems(1)?;
                                        Ok(ZerepochScripts::deserialize(raw)?)
                                    })().map_err(|e| e.annotate("zerepoch_scripts"))?);
                                },
                ...
                // sophie jen format (still valid for aurum)
                CBORType::Array => {
                    let len = raw.array()?;
                    let mut read_len = CBORReadLen::new(len);
                ...
                // sophie pre-jen format (still valid for aurum + jen)
...
mod tests {
    ...
    fn metadata_serialize() {
        ...
        // aurum
        let mut zerepoch_scripts = ZerepochScripts::new();
        zerepoch_scripts.add(&ZerepochScript::new([61u8; 29].to_vec()));
        aux_data.set_zerepoch_scripts(&zerepoch_scripts);
