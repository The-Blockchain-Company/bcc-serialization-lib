...
impl cbor_event::se::Serialize for TransactionWitnessSet {
    fn serialize<'se, W: Write>(&self, serializer: &'se mut Serializer<W>) -> cbor_event::Result<&'se mut Serializer<W>> {
        serializer.write_map(cbor_event::Len::Len(match &self.vkeys { Some(_) => 1, None => 0 } + match &self.native_scripts { Some(_) => 1, None => 0 } + match &self.bootstraps { Some(_) => 1, None => 0 } + match &self.zerepoch_scripts { Some(_) => 1, None => 0 } + match &self.zerepoch_data { Some(_) => 1, None => 0 } + match &self.redeemers { Some(_) => 1, None => 0 }))?;
        ...
        if let Some(field) = &self.zerepoch_scripts {
            serializer.write_unsigned_integer(3)?;
            field.serialize(serializer)?;
        }
        if let Some(field) = &self.zerepoch_data {
            serializer.write_unsigned_integer(4)?;
            field.serialize(serializer)?;
        }
            ...
            while match len { cbor_event::Len::Len(n) => read < n as usize, cbor_event::Len::Indefinite => true, } {
                match raw.cbor_type()? {
                    CBORType::UnsignedInteger => match raw.unsigned_integer()? {
                        ...
                        4 =>  {
                            if zerepoch_data.is_some() {
                                return Err(DeserializeFailure::DuplicateKey(Key::Uint(4)).into());
                            }
                            zerepoch_data = Some((|| -> Result<_, DeserializeError> {
                                read_len.read_elems(1)?;
                                Ok(ZerepochList::deserialize(raw)?)
                            })().map_err(|e| e.annotate("zerepoch_data"))?);
                        },
                        5 =>  {
