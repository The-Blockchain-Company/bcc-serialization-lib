...
fn fake_full_tx(tx_builder: &TransactionBuilder, body: TransactionBody) -> Result<Transaction, JsError> {
    ...
    let bootstrap_keys = match tx_builder.input_types.bootstraps.len() {
        ...
        _x => {
            ...
            for addr in &tx_builder.input_types.bootstraps {
                // picking icarus over klarity for fake witness generation shouldn't matter
                result.add(&make_icarus_bootstrap_witness(
                    &TransactionHash::from([0u8; TransactionHash::BYTE_COUNT]),
                    &ColeAddress::from_bytes(addr.clone()).unwrap(),
                    &fake_key_root
                ));
            }
            Some(result)
    ...
    let witness_set = TransactionWitnessSet {
        ...
        // TODO: zerepoch support?
        zerepoch_scripts: None,
        zerepoch_data: None,
        redeemers: None,
    };
    Ok(Transaction {
...
pub enum CoinSelectionStrategyCIP2 {
    /// Performs CIP2's Largest First bcc-only selection. Will error if outputs contain non-BCC assets.
    LargestFirst,
    /// Performs CIP2's Random Improve bcc-only selection. Will error if outputs contain non-BCC assets.
    RandomImprove,
    /// Same as LargestFirst, but before adding BCC, will insert by largest-first for each asset type.
    LargestFirstMultiAsset,
    /// Same as RandomImprove, but before adding BCC, will insert by random-improve for each asset type.
...
impl TransactionBuilder {
    ...
    pub fn add_bootstrap_input(&mut self, hash: &ColeAddress, input: &TransactionInput, amount: &Value) {
        self.inputs.push(TxBuilderInput {
            input: input.clone(),
            amount: amount.clone(),
        });
    ...
    pub fn add_input(&mut self, address: &Address, input: &TransactionInput, amount: &Value) {
        ...
        match &ColeAddress::from_address(address) {
            Some(addr) => {
    ...
    /// It might contain some metadata plus native or Zerepoch scripts
            ...
            Some(Ordering::Greater) => {
                ...
                if has_assets(change_estimator.multiasset()) {
                    fn will_adding_asset_make_output_overflow(output: &TransactionOutput, current_assets: &Assets, asset_to_add: (PolicyID, AssetName, BigNum), max_value_size: u32, coins_per_utxo_word: &Coin) -> bool {
                        ...
                        // calculate minBCC for more precise max value size
                    ...
                    fn pack_nfts_for_change(max_value_size: u32, coins_per_utxo_word: &Coin, change_address: &Address, change_estimator: &Value, data_hash: Option<DataHash>) -> Result<Vec<MultiAsset>, JsError> {
                        // we insert the entire available BCC temporarily here since that could potentially impact the size
                    ...
                    while let Some(Ordering::Greater) = change_left.multiasset.as_ref().map_or_else(|| None, |ma| ma.partial_cmp(&MultiAsset::new())) {
                        ...
                        for nft_change in nft_changes.iter() {
                            ...
                            if change_left.coin() < min_ada.checked_add(&new_fee)? {
                                return Err(JsError::from_str("Not enough BCC leftover to include non-BCC assets in a change address"));
                            }
                    ...
                    // add potentially a separate pure BCC change output
    ...
    fn build_and_size(&self) -> Result<(TransactionBody, usize), JsError> {
        ...
        let built = TransactionBody {
            ...
            // TODO: update for use with Aurum
    ...
    /// warning: min_fee may be slightly larger than the actual minimum fee (ex: a few entropics)
...
mod tests {
    ...
    fn cole_address() -> Address {
        ColeAddress::from_base58("Ae2tdPwUPEZ5uzkzh1o2DHECiUi3iugvnnKHRisPgRRP3CTF4KCMvy54Xd3").unwrap().to_address()
    }
    ...
    fn build_tx_exact_change() {
        // transactions where we have exactly enough BCC to add change should pass
        let mut tx_builder = create_tx_builder_with_fee(&create_linear_fee(0, 0));
        let spend = root_key_15()
            .derive(harden(1852))
            .derive(harden(1815))
    ...
    fn build_tx_with_native_assets_change() {
        ...
        for (multiasset, bcc) in multiassets
            .iter()
            .zip([100u64, 100].iter().cloned().map(to_bignum))
        {
            let mut input_amount = Value::new(&bcc);
    ...
    fn build_tx_burn_less_than_min_ada() {
        ...
        let output_addr = ColeAddress::from_base58("Ae2tdPwUPEZD9QQf2ZrcYV34pYJwxK4vqXaF8EXkup1eYH73zUScHReM42b").unwrap();
        tx_builder.add_output(
            &TransactionOutputBuilder::new()
        ...
        let change_addr = ColeAddress::from_base58("Ae2tdPwUPEZGUEsuMAhvDcy94LKsZxDjCbgaiBBMgYpR8sKf96xJmit7Eho").unwrap();
        let added_change = tx_builder.add_change_if_needed(
    ...
    fn build_tx_no_useless_multiasset() {
        ...
        // add an output that uses up all the token but leaves BCC
        ...
        // add an input that contains an asset & BCC
        ...
        // the change should be only BCC and not have any multiasset struct in it
    ...
    fn build_tx_add_change_split_nfts() {
        ...
        let output_addr = ColeAddress::from_base58("Ae2tdPwUPEZD9QQf2ZrcYV34pYJwxK4vqXaF8EXkup1eYH73zUScHReM42b").unwrap().to_address();
        ...
        let change_addr = ColeAddress::from_base58("Ae2tdPwUPEZGUEsuMAhvDcy94LKsZxDjCbgaiBBMgYpR8sKf96xJmit7Eho").unwrap().to_address();
    ...
    fn tx_builder_cip2_largest_first_multiasset() {
        ...
        // should be taken to get enough BCC
    ...
    fn set_metadata_with_empty_auxiliary() {
        ...
        assert!(aux.zerepoch_scripts().is_none());
