...
// Specifies an amount of BCC in terms of entropic
...
#[wasm_bindgen]
pub fn make_klarity_bootstrap_witness(
    tx_body_hash: &TransactionHash,
    addr: &ColeAddress,
    key: &LegacyKlarityPrivateKey,
) -> BootstrapWitness {
...
#[wasm_bindgen]
pub fn hash_zerepoch_data(zerepoch_data: &ZerepochData) -> DataHash {
    DataHash::from(blake2b256(&zerepoch_data.to_bytes()))
}
#[wasm_bindgen]
pub fn hash_script_data(redeemers: &Redeemers, cost_models: &Costmdls, datums: Option<ZerepochList>) -> ScriptDataHash {
    let mut buf = Vec::new();
...
fn bundle_size(
    ...
    // based on https://github.com/The-Blockchain-Company/bcc-ledger-specs/blob/master/doc/explanations/min-utxo-aurum.rst
...
pub fn min_ada_required(
    ...
    coins_per_utxo_word: &BigNum, // protocol parameter (in entropic)
...
/// Bcc Wallet and Node styles are supported.
///
/// * wallet: https://github.com/The-Blockchain-Company/bcc-wallet/blob/master/specifications/api/swagger.yaml
/// * node: https://github.com/The-Blockchain-Company/bcc-node/blob/master/doc/reference/simple-scripts.md
///
...
mod tests {
    ...
    // taken from https://github.com/The-Blockchain-Company/bcc-ledger-specs/blob/master/doc/explanations/min-utxo-aurum.rst
    ...
    fn correct_script_data_hash() {
        let mut datums = ZerepochList::new();
        datums.add(&ZerepochData::new_integer(&BigInt::from_str("1000").unwrap()));
        let mut redeemers = Redeemers::new();
        redeemers.add(&Redeemer::new(&RedeemerTag::new_spend(), &BigNum::from_str("1").unwrap(), &ZerepochData::new_integer(&BigInt::from_str("2000").unwrap()), &ExUnits::new(&BigNum::from_str("0").unwrap(), &BigNum::from_str("0").unwrap())));
        let zerepoch_cost_model = CostModel::from_bytes(vec![
        ...
        cost_models.insert(&Language::new_zerepoch_v1(), &zerepoch_cost_model);
