...
// https://github.com/The-Blockchain-Company/cddl-codegen
...
pub struct ZerepochScript(Vec<u8>);

to_from_bytes!(ZerepochScript);

#[wasm_bindgen]
impl ZerepochScript {
    /**
     * Creates a new Zerepoch script from the RAW bytes of the compiled script.
     * This does NOT include any CBOR encoding around these bytes (e.g. from "cborBytes" in bcc-cli)
     * If you creating this from those you should use ZerepochScript::from_bytes() instead.
     */
    pub fn new(bytes: Vec<u8>) -> ZerepochScript {
        Self(bytes)
    }

    /**
     * The raw bytes of this compiled Zerepoch script.
     * If you need "cborBytes" for bcc-cli use ZerepochScript::to_bytes() instead.
     */
...
#[wasm_bindgen]
#[derive(Clone, Debug, Eq, Ord, PartialEq, PartialOrd)]
pub struct ZerepochScripts(Vec<ZerepochScript>);

to_from_bytes!(ZerepochScripts);

#[wasm_bindgen]
impl ZerepochScripts {
    pub fn new() -> Self {
        Self(Vec::new())
    }
    ...
    pub fn get(&self, index: usize) -> ZerepochScript {
        self.0[index].clone()
    }

    pub fn add(&mut self, elem: &ZerepochScript) {
...
#[wasm_bindgen]
#[derive(Clone, Debug, Eq, Ord, PartialEq, PartialOrd)]
pub struct ConstrZerepochData {
    alternative: BigNum,
    data: ZerepochList,
}

to_from_bytes!(ConstrZerepochData);

#[wasm_bindgen]
impl ConstrZerepochData {
    pub fn alternative(&self) -> BigNum {
        self.alternative.clone()
    }

    pub fn data(&self) -> ZerepochList {
        self.data.clone()
    }

    pub fn new(alternative: &BigNum, data: &ZerepochList) -> Self {
        Self {
...
impl ConstrZerepochData {
    // see: https://github.com/The-Blockchain-Company/zerepoch/blob/1f31e640e8a258185db01fa899da63f9018c0e85/zerepoch-core/zerepoch-core/src/ZerepochCore/Data.hs#L61
...
impl Costmdls {
    ...
    pub(crate) fn language_views_encoding(&self) -> Vec<u8> {
        ...
        for (key, key_bytes) in keys_bytes.iter() {
            ...
            // Due to a bug in the bcc-node The-Blockchain-Company/bcc-ledger-specs/issues/2512
...
#[wasm_bindgen]
impl Language {
    pub fn new_zerepoch_v1() -> Self {
        Self(LanguageKind::ZerepochV1)
    }
...
#[wasm_bindgen]
#[derive(Clone, Debug, Eq, Ord, PartialEq, PartialOrd)]
pub struct ZerepochMap(std::collections::BTreeMap<ZerepochData, ZerepochData>);

to_from_bytes!(ZerepochMap);
...
impl ZerepochMap {
    ...
    pub fn insert(&mut self, key: &ZerepochData, value: &ZerepochData) -> Option<ZerepochData> {
        self.0.insert(key.clone(), value.clone())
    }

    pub fn get(&self, key: &ZerepochData) -> Option<ZerepochData> {
        self.0.get(key).map(|v| v.clone())
    }

    pub fn keys(&self) -> ZerepochList {
        ZerepochList {
...
#[derive(Clone, Debug, Eq, Ord, PartialEq, PartialOrd)]
enum ZerepochDataEnum {
    ConstrZerepochData(ConstrZerepochData),
    Map(ZerepochMap),
    List(ZerepochList),
    Integer(BigInt),
    Bytes(Vec<u8>),
...
#[wasm_bindgen]
#[derive(Clone, Debug, Eq, Ord, PartialEq, PartialOrd)]
pub struct ZerepochData {
    datum: ZerepochDataEnum,
...
to_from_bytes!(ZerepochData);

#[wasm_bindgen]
impl ZerepochData {
    pub fn new_constr_zerepoch_data(constr_zerepoch_data: &ConstrZerepochData) -> Self {
        Self {
            datum: ZerepochDataEnum::ConstrZerepochData(constr_zerepoch_data.clone()),
            original_bytes: None,
        }
    }

    pub fn new_map(map: &ZerepochMap) -> Self {
        Self {
            datum: ZerepochDataEnum::Map(map.clone()),
            original_bytes: None,
        }
    }

    pub fn new_list(list: &ZerepochList) -> Self {
        Self {
            datum: ZerepochDataEnum::List(list.clone()),
            original_bytes: None,
        }
    }
    ...
    pub fn new_integer(integer: &BigInt) -> Self {
        Self {
            datum: ZerepochDataEnum::Integer(integer.clone()),
            original_bytes: None,
        }
    }
    ...
    pub fn new_bytes(bytes: Vec<u8>) -> Self {
        Self {
            datum: ZerepochDataEnum::Bytes(bytes),
            original_bytes: None,
        }
    }

    pub fn kind(&self) -> ZerepochDataKind {
        match &self.datum {
            ZerepochDataEnum::ConstrZerepochData(_) => ZerepochDataKind::ConstrZerepochData,
            ZerepochDataEnum::Map(_) => ZerepochDataKind::Map,
            ZerepochDataEnum::List(_) => ZerepochDataKind::List,
            ZerepochDataEnum::Integer(_) => ZerepochDataKind::Integer,
            ZerepochDataEnum::Bytes(_) => ZerepochDataKind::Bytes,
        }
    }

    pub fn as_constr_zerepoch_data(&self) -> Option<ConstrZerepochData> {
        match &self.datum {
            ZerepochDataEnum::ConstrZerepochData(x) => Some(x.clone()),
            _ => None,
        }
    }

    pub fn as_map(&self) -> Option<ZerepochMap> {
        match &self.datum {
            ZerepochDataEnum::Map(x) => Some(x.clone()),
            _ => None,
        }
    }

    pub fn as_list(&self) -> Option<ZerepochList> {
        match &self.datum {
            ZerepochDataEnum::List(x) => Some(x.clone()),
            _ => None,
        }
    }
    ...
    pub fn as_integer(&self) -> Option<BigInt> {
        match &self.datum {
            ZerepochDataEnum::Integer(x) => Some(x.clone()),
            _ => None,
        }
    }
    ...
    pub fn as_bytes(&self) -> Option<Vec<u8>> {
        match &self.datum {
            ZerepochDataEnum::Bytes(x) => Some(x.clone()),
            _ => None,
        }
    }
}
...
#[wasm_bindgen]
#[derive(Clone, Debug, Eq, Ord, PartialEq, PartialOrd)]
pub struct ZerepochList {
    elems: Vec<ZerepochData>,
    // We should always preserve the original datums when deserialized as this is NOT canonicized
    // before computing datum hashes. This field will default to bcc-cli behavior if None
    // and will re-use the provided one if deserialized, unless the list is modified.
    definite_encoding: Option<bool>,
}

to_from_bytes!(ZerepochList);
...
impl ZerepochList {
    ...
    pub fn get(&self, index: usize) -> ZerepochData {
        self.elems[index].clone()
    }

    pub fn add(&mut self, elem: &ZerepochData) {
...
impl Redeemer {
    ...
    pub fn new(tag: &RedeemerTag, index: &BigNum, data: &ZerepochData, ex_units: &ExUnits) -> Self {
        Self {
            tag: tag.clone(),
            index: index.clone(),
            data: data.clone(),
...
impl cbor_event::se::Serialize for ZerepochScript {
    fn serialize<'se, W: Write>(&self, serializer: &'se mut Serializer<W>) -> cbor_event::Result<&'se mut Serializer<W>> {
        serializer.write_bytes(&self.0)
    }
}

impl Deserialize for ZerepochScript {
    fn deserialize<R: BufRead + Seek>(raw: &mut Deserializer<R>) -> Result<Self, DeserializeError> {
        Ok(Self(raw.bytes()?))
    }
}

impl cbor_event::se::Serialize for ZerepochScripts {
    fn serialize<'se, W: Write>(&self, serializer: &'se mut Serializer<W>) -> cbor_event::Result<&'se mut Serializer<W>> {
...
impl Deserialize for ZerepochScripts {
    fn deserialize<R: BufRead + Seek>(raw: &mut Deserializer<R>) -> Result<Self, DeserializeError> {
        let mut arr = Vec::new();
        (|| -> Result<_, DeserializeError> {
            ...
            while match len { cbor_event::Len::Len(n) => arr.len() < n as usize, cbor_event::Len::Indefinite => true, } {
                ...
                arr.push(ZerepochScript::deserialize(raw)?);
            }
            Ok(())
        })().map_err(|e| e.annotate("ZerepochScripts"))?;
        Ok(Self(arr))
    }
}
...
impl cbor_event::se::Serialize for ConstrZerepochData {
    fn serialize<'se, W: Write>(&self, serializer: &'se mut Serializer<W>) -> cbor_event::Result<&'se mut Serializer<W>> {
...
impl Deserialize for ConstrZerepochData {
    fn deserialize<R: BufRead + Seek>(raw: &mut Deserializer<R>) -> Result<Self, DeserializeError> {
        (|| -> Result<_, DeserializeError> {
            let (alternative, data) = match raw.tag()? {
                ...
                Self::GENERAL_FORM_TAG => {
                    ...
                    let data = (|| -> Result<_, DeserializeError> {
                        Ok(ZerepochList::deserialize(raw)?)
                ...
                tag => {
                    if let Some(alternative) = Self::compact_cbor_tag_to_alternative(tag) {
                        (to_bignum(alternative), ZerepochList::deserialize(raw)?)
                    } else {
            ...
            Ok(ConstrZerepochData{
                alternative,
                data,
            })
        })().map_err(|e| e.annotate("ConstrZerepochData"))
    }
}
...
impl cbor_event::se::Serialize for ZerepochMap {
    fn serialize<'se, W: Write>(&self, serializer: &'se mut Serializer<W>) -> cbor_event::Result<&'se mut Serializer<W>> {
        serializer.write_map(cbor_event::Len::Len(self.0.len() as u64))?;
        for (key, value) in &self.0 {
            key.serialize(serializer)?;
...
impl Deserialize for ZerepochMap {
    fn deserialize<R: BufRead + Seek>(raw: &mut Deserializer<R>) -> Result<Self, DeserializeError> {
        let mut table = std::collections::BTreeMap::new();
        (|| -> Result<_, DeserializeError> {
            let len = raw.map()?;
            while match len { cbor_event::Len::Len(n) => table.len() < n as usize, cbor_event::Len::Indefinite => true, } {
                ...
                let key = ZerepochData::deserialize(raw)?;
                let value = ZerepochData::deserialize(raw)?;
                if table.insert(key.clone(), value).is_some() {
                    return Err(DeserializeFailure::DuplicateKey(Key::Str(String::from("some complicated/unsupported type"))).into());
                }
            }
            Ok(())
        })().map_err(|e| e.annotate("ZerepochMap"))?;
        Ok(Self(table))
    }
}

impl cbor_event::se::Serialize for ZerepochDataEnum {
    fn serialize<'se, W: Write>(&self, serializer: &'se mut Serializer<W>) -> cbor_event::Result<&'se mut Serializer<W>> {
        match self {
            ZerepochDataEnum::ConstrZerepochData(x) => {
                x.serialize(serializer)
            },
            ZerepochDataEnum::Map(x) => {
                x.serialize(serializer)
            },
            ZerepochDataEnum::List(x) => {
                x.serialize(serializer)
            },
            ZerepochDataEnum::Integer(x) => {
                x.serialize(serializer)
            },
            ZerepochDataEnum::Bytes(x) => {
...
impl Deserialize for ZerepochDataEnum {
    fn deserialize<R: BufRead + Seek>(raw: &mut Deserializer<R>) -> Result<Self, DeserializeError> {
        (|| -> Result<_, DeserializeError> {
            let initial_position = raw.as_mut_ref().seek(SeekFrom::Current(0)).unwrap();
            match (|raw: &mut Deserializer<_>| -> Result<_, DeserializeError> {
                Ok(ConstrZerepochData::deserialize(raw)?)
            })(raw)
            {
                Ok(variant) => return Ok(ZerepochDataEnum::ConstrZerepochData(variant)),
                Err(_) => raw.as_mut_ref().seek(SeekFrom::Start(initial_position)).unwrap(),
            };
            match (|raw: &mut Deserializer<_>| -> Result<_, DeserializeError> {
                Ok(ZerepochMap::deserialize(raw)?)
            })(raw)
            {
                Ok(variant) => return Ok(ZerepochDataEnum::Map(variant)),
                Err(_) => raw.as_mut_ref().seek(SeekFrom::Start(initial_position)).unwrap(),
            };
            match (|raw: &mut Deserializer<_>| -> Result<_, DeserializeError> {
                Ok(ZerepochList::deserialize(raw)?)
            ...
            {
                Ok(variant) => return Ok(ZerepochDataEnum::List(variant)),
                Err(_) => raw.as_mut_ref().seek(SeekFrom::Start(initial_position)).unwrap(),
            };
            match (|raw: &mut Deserializer<_>| -> Result<_, DeserializeError> {
            ...
            {
                Ok(variant) => return Ok(ZerepochDataEnum::Integer(variant)),
                Err(_) => raw.as_mut_ref().seek(SeekFrom::Start(initial_position)).unwrap(),
            };
            match (|raw: &mut Deserializer<_>| -> Result<_, DeserializeError> {
            ...
            {
                Ok(variant) => return Ok(ZerepochDataEnum::Bytes(variant)),
                Err(_) => raw.as_mut_ref().seek(SeekFrom::Start(initial_position)).unwrap(),
            };
            Err(DeserializeError::new("ZerepochDataEnum", DeserializeFailure::NoVariantMatched.into()))
        })().map_err(|e| e.annotate("ZerepochDataEnum"))
    }
}

impl cbor_event::se::Serialize for ZerepochData {
    fn serialize<'se, W: Write>(&self, serializer: &'se mut Serializer<W>) -> cbor_event::Result<&'se mut Serializer<W>> {
...
impl cbor_event::se::Serialize for ZerepochList {
    fn serialize<'se, W: Write>(&self, serializer: &'se mut Serializer<W>) -> cbor_event::Result<&'se mut Serializer<W>> {
...
impl Deserialize for ZerepochList {
    fn deserialize<R: BufRead + Seek>(raw: &mut Deserializer<R>) -> Result<Self, DeserializeError> {
        let mut arr = Vec::new();
        let len = (|| -> Result<_, DeserializeError> {
            ...
            while match len { cbor_event::Len::Len(n) => arr.len() < n as usize, cbor_event::Len::Indefinite => true, } {
                ...
                arr.push(ZerepochData::deserialize(raw)?);
            }
            Ok(len)
        })().map_err(|e| e.annotate("ZerepochList"))?;
        Ok(Self {
            elems: arr,
...
impl Deserialize for Redeemer {
    fn deserialize<R: BufRead + Seek>(raw: &mut Deserializer<R>) -> Result<Self, DeserializeError> {
        (|| -> Result<_, DeserializeError> {
            ...
            let data = (|| -> Result<_, DeserializeError> {
                Ok(ZerepochData::deserialize(raw)?)
...
mod tests {
    ...
    #[test]
    pub fn zerepoch_constr_data() {
        let constr_0 = ZerepochData::new_constr_zerepoch_data(
            &ConstrZerepochData::new(&to_bignum(0), &ZerepochList::new())
        );
        let constr_0_hash = hex::encode(hash_zerepoch_data(&constr_0).to_bytes());
        assert_eq!(constr_0_hash, "923918e403bf43c34b4ef6b48eb2ee04babed17320d8d1b9ff9ad086e86f44ec");
        let constr_0_roundtrip = ZerepochData::from_bytes(constr_0.to_bytes()).unwrap();
        // TODO: do we want semantic equality or bytewise equality?
        //assert_eq!(constr_0, constr_0_roundtrip);
        let constr_1854 = ZerepochData::new_constr_zerepoch_data(
            &ConstrZerepochData::new(&to_bignum(1854), &ZerepochList::new())
        );
        let constr_1854_roundtrip = ZerepochData::from_bytes(constr_1854.to_bytes()).unwrap();
        //assert_eq!(constr_1854, constr_1854_roundtrip);
    }

    #[test]
    pub fn zerepoch_list_serialization_cli_compatibility() {
        // mimic bcc-cli array encoding, see https://github.com/The-Blockchain-Company/bcc-serialization-lib/issues/227
        ...
        let datum = ZerepochData::from_bytes(Vec::from_hex(datum_cli).unwrap()).unwrap();
        assert_eq!(datum_cli, hex::encode(datum.to_bytes()));

        // encode empty arrays as fixed
        assert_eq!("80", hex::encode(ZerepochList::new().to_bytes()));

        // encode arrays as indefinite length array
        let mut list = ZerepochList::new();
        list.add(&ZerepochData::new_integer(&BigInt::from_str("1").unwrap()));
        assert_eq!("9f01ff", hex::encode(list.to_bytes()));

        // witness_set should have fixed length array
        let mut witness_set = TransactionWitnessSet::new();
        witness_set.set_zerepoch_data(&list);
        assert_eq!("a1049f01ff", hex::encode(witness_set.to_bytes()));

        list = ZerepochList::new();
        list.add(&datum);
        witness_set.set_zerepoch_data(&list);
    ...
    pub fn test_cost_model() {
        ...
        cms.insert(&Language::new_zerepoch_v1(), &cm);
        assert_eq!(
